...
!-----------------------------------------------------------------
!     #####################
      SUBROUTINE COUPL_TOPD_SUB (DEC, DC, DMI, PMESH_SIZE, IO, S, K, NK, NP, NPE, &
...
!!    ------
!!
!!      B. Vincendon from coupl_topd.F90
!!
!!    MODIFICATIONS
!!    -------------
!!
...
!
USE MODD_TOPODYN,        ONLY : NNCAT, NMESHT, NNMC, XMPARA, XDMAXT,XQTOT
!
USE MODD_COUPLING_TOPD,  ONLY : XWG_FULL, XDTOPI, XKAC_PRE, XDTOPT, XWTOPT, XWSTOPT, XAS_NATURE,&
                                  ...
                                  NFREQ_MAPS_ASAT,XAVG_DRAINCM,XRAINFALLCM,XAVG_HORTCM,&
                                  LBUDGET_TOPD, LPERT_PARAM, LPERT_INIT,NNBV_IN_MESH, &
                                  LWRITE_SEVERITY_MAPS 
!
USE MODD_CSTS,             ONLY : XRHOLW
USE MODD_SURF_PAR,         ONLY : XUNDEF, NUNDEF
USE MODD_ISBA_PAR,         ONLY : XWGMIN

USE MODD_DUMMY_EXP_PROFILE,ONLY :XF_PARAM
!
USE MODI_GET_LUOUT
...
REAL, DIMENSION(U%NDIM_FULL)           :: ZWGI_FULL
...
INTEGER                       :: JCAT,JSUB,JSCAT    ! loop control
INTEGER                       :: ILUOUT          ! unit number of listing file
INTEGER                       :: IACT_GROUND_LAYER, IMASK, ISUM
REAL                          :: ZWORK, ZTMP_Q2
!
...
!-------------------------------------------------------------------------------
IF (LHOOK) CALL DR_HOOK('COUPL_TOPD_SUB',0,ZHOOK_HANDLE)
!
...
!               ----------------------------------------
!
CALL TOPODYN_LAT_SUB(ZRT(:,:),ZDEFT(:,:),ZKAPPA(:,:),ZKAPPAC(:),GTOPD)
!
...
! Full grid
DO JJ=1,NNCAT
  WHERE ( XDTOPT(JJ,:)/=XUNDEF .AND. XDTOPT(JJ,:)/=0. .AND. &
          ZDEFT(JJ,:)/=XUNDEF .AND. ZRI_WGIT(JJ,:)/=XUNDEF)
...
!! on utilise xavg_runoffc car les patches y sont pris en compte
!! on ne multiply pas par XATOP car s'est déjà fait dans hydro_sgh.f90
ZWOVSATI_F(:) = 0.
...
DO JJ=1,NNCAT
  WHERE (ZRUNOFF_IBV_F(:,JJ)/=XUNDEF.AND.UG%G%XMESH_SIZE(:)/=XUNDEF)
    ZRUNOFF_IBV_F(:,JJ)=ZRUNOFF_IBV_F(:,JJ)*UG%G%XMESH_SIZE(:)/XRHOLW/3600.!/I%XRUNOFFD(:,1)! from kg/m2 to m3
  ENDWHERE
ENDDO
!
...
!
CALL ROUTING_SUB(ZRUNOFF_TOPD,ZDRAIN_TOPD,KSTEP)
...
ZTMP_Q2=250.
DO JCAT=1,NNCAT
  IF (XQTOT(JCAT,KSTEP)>ZTMP_Q2.AND.LWRITE_SEVERITY_MAPS)  THEN
   CALL WRITE_ALERT_MAP(JCAT,KSTEP,HSTEP)
  ENDIF
ENDDO
...
IF (NFREQ_MAPS_ASAT/=0.AND.MOD(KSTEP,NFREQ_MAPS_ASAT)==0) THEN
  ...
  CALL WRITE_FILE_ISBAMAP(UG,NUNIT,ZASI_F,U%NDIM_FULL)
  CALL CLOSE_FILE('ASCII ',NUNIT)
ENDIF
!
IF (LHOOK) CALL DR_HOOK('COUPL_TOPD_SUB',1,ZHOOK_HANDLE)
!
END SUBROUTINE COUPL_TOPD_SUB
