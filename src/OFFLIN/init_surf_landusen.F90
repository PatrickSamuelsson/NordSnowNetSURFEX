...
!#############################################################
SUBROUTINE INIT_SURF_LANDUSE_n (DTCO, OREAD_BUDGETC, U, UG, IM, SV, SLT,  &
                               BLOWSNW, NDST, DST,                        &
...
!       
IF ( HINIT == 'PRE' ) THEN ! IF 'PRE': KDAY and KMONTH not define. (Only here for irrigation)
  NDAY = 1
  NMONTH = 1
...
ENDIF
 CALL COMPUTE_ISBA_PARAMETERS(DTCO, OREAD_BUDGETC, UG, U,                    &
                              ...
                              IM%NGB,NDST, DST, SLT, BLOWSNW, SV, HPROGRAM, HINIT,&
                              OLAND_USE,ILU,NMONTH,NDAY,KSV,KSW,HSV,PCO2, PRHOA, &
