...
!!      Original    06/2003
!
USE MODN_IO_OFFLINE, ONLY : CINTERP_SW
!
! ###########################################################################
...
REAL, DIMENSION(:),INTENT(INOUT) :: PDIR_SW1,PDIR_SW2,PSCA_SW1,PSCA_SW2,PLW1,PLW2
...
REAL, DIMENSION(:),INTENT(IN) :: PZEN1,PSUMZEN,PZEN_INT,PZEN2
! ###########################################################################
...
DO J = 1,SIZE(PTA1)
  ...
  IF (PTA1(J)/=XUNDEF) THEN
    ...
    CASE ('ZEN')
      ...
      ENDIF
      !
      PDIR_SW1(J) = MAX(0.0,PDIR_SW1(J))
      PSCA_SW1(J) = MAX(0.0,PSCA_SW1(J))
      !
      ZSW2 = PDIR_SW2(J) + PSCA_SW2(J)
      !
      ...
      ENDIF   
      !
      PDIR_SW2(J) = MAX(0.0,PDIR_SW2(J))
      PSCA_SW2(J) = MAX(0.0,PSCA_SW2(J))     
      !      
      ...
      IF (ZSW1 .LT. XSURF_EPSILON) THEN
         ZF_SCA_SW1 = 1.
      ELSE
         ZF_SCA_SW1 = PSCA_SW1(J) / ZSW1
      ENDIF
      !
      IF (ZSW2 .LT. XSURF_EPSILON) THEN
         ZF_SCA_SW2 = 1.
      ELSE
         ZF_SCA_SW2 = PSCA_SW2(J) / ZSW2
      ENDIF
      !
      !
    ...
    CASE ('OLD')
      !
      ZCOEF2=0.
      IF (PSUMZEN(J)>0.) ZCOEF2=MAX((COS(PZEN_INT(J))/PSUMZEN(J)),0.)
      !
      XDIR_SW(J,1) = MIN(PDIR_SW2(J)*ZCOEF2,1300.0*MAX(COS(PZEN_INT(J)),0.))
      !
      XSCA_SW(J,1) = MIN(PSCA_SW2(J)*ZCOEF2,1300.0*MAX(COS(PZEN_INT(J)),0.))
    ...
    CASE DEFAULT
      CALL ABOR1_SFX('OPTION "'//CINTERP_SW//'" NOT RECOGNIZED FOR CINTERP_SW in NAM_IO_OFFLINE')
    END SELECT
    !
    !
    XRAIN (J)= PRAIN2(J)
