...
IF (NRANK==NPIO) THEN
  ...
  YSC%IM%S%TTIME = YSC%U%TTIME
  YSC%SM%S%TTIME = YSC%U%TTIME
  YSC%WM%W%TTIME = YSC%U%TTIME
  YSC%TM%TOP%TTIME = YSC%U%TTIME
  IDATEF(1)=YSC%U%TTIME%TDATE%YEAR
  IDATEF(2)=YSC%U%TTIME%TDATE%MONTH
  IDATEF(3)=YSC%U%TTIME%TDATE%DAY
  ZTIME = YSC%U%TTIME%TIME
  IDATEF(4)=INT(YSC%U%TTIME%TIME/3600.) 
  IDATEF(5)=INT(YSC%U%TTIME%TIME/60.) - IDATEF(4) * 60 
    IDATEF(6)=1 
    IDATEF(7:11)=0  
