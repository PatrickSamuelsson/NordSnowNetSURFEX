...
REAL, DIMENSION(KI),    INTENT(OUT), OPTIONAL :: PSEA_UCU     ! sea u-current            (m/s)
REAL, DIMENSION(KI),    INTENT(OUT), OPTIONAL :: PSEA_VCU     ! sea v-current            (m/s)
...
REAL, DIMENSION(KI) :: ZSEA_UCU      ! Sea u-current (m/s)
REAL, DIMENSION(KI) :: ZSEA_VCU      ! Sea v-current (m/s)
...
REAL, DIMENSION(KI) :: ZWAVE_CHA     ! Charnock coeff (-)
REAL, DIMENSION(KI) :: ZWAVE_HS      ! Wave significant height (m)
REAL, DIMENSION(KI) :: ZWAVE_TP      ! Wave peak period (s)
REAL, DIMENSION(KI) :: ZWAVE_UCU     ! Sea u-current from wave model (m/s)
REAL, DIMENSION(KI) :: ZWAVE_VCU     ! Sea v-current from wave model (m/s)
!
REAL                :: ZTIME_CPL
!
LOGICAL             :: GRECV_LAND
...
!
IF(.NOT.(GRECV_LAND.OR.GRECV_SEA.OR.GRECV_WAVE))THEN
...
ENDIF
!
IF(PRESENT(PSEA_UCU).AND.PRESENT(PSEA_VCU))THEN
  PSEA_UCU(:)=ZSEA_UCU(:)
  PSEA_VCU(:)=ZSEA_VCU(:)
ENDIF
!
!-------------------------------------------------------------------------------
! Put variable for waves
!-------------------------------------------------------------------------------
!
IF(GRECV_WAVE)THEN
  CALL PUT_SFX_WAVE(S, U, ILUOUT, ZWAVE_CHA(:), ZWAVE_UCU(:), ZWAVE_VCU(:), &
                    ZWAVE_HS(:), ZWAVE_TP(:) )
ENDIF
!
